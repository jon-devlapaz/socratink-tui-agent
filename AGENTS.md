...
- `LOOP_APP_VERSION_DEFAULT` in `lib/loop-server/version.mjs` is the canonical loop chrome label (`/health` → `app_version`). **CI auto-bumps on PRs to `main`**. Smoke CI syncs `LOOP_APP_VERSION` to Railway after merge; optional `LOOP_APP_VERSION` in `.env` overrides locally.
...
- Smoke CI sets Railway vars with `railway variable set --skip-deploys --project …` (no `railway link`); uses `--skip-deploys` to avoid backboard timeouts. Railway GitHub connection builds/deploys from `main`.
