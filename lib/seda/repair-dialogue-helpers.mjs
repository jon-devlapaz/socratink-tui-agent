...
export function uncertaintyRecoveryPrompt(repairScaffold, step) {
  const missingOp = repairScaffold.missing_operation;
  if (step === 1) {
    return `That's okay. In one sentence, explain the key change (${missingOp}) in your own words.`;
  }
  return `Last try before we pause: give 3 keywords for the key change (${missingOp}), then write one sentence using them.`;
}
...
export function repairNudge(turnIndex, state, judge) {
  ...
  if (state?.escalationLevel >= 2 || turnIndex >= 4) {
    const finalPass = [
      ...
      "Final try: one sentence only, before -> operation -> after.",
      "One more concise attempt, then we pause and redrill.",
    ];
    return `${finalPass[(turnIndex - 1) % finalPass.length]} ${emphasis}`;
  }
  if (state?.escalationLevel >= 1) {
    const analogical = [
      "Try a concrete analogy for the missing step.",
      "Switch modality: describe the missing step with a simple parallel.",
      "Use one familiar comparison to express the missing operation.",
    ];
...
export function missingOperationFeedback(repairScaffold, judge) {
  if (judge?.bridge_ready) return "";
  const missing = repairScaffold?.missing_operation || "the missing operation";
  const before = repairScaffold?.before || "the before-state";
  const after = repairScaffold?.after || "the after-state";
  return [
    `Still not bridge-ready: missing operation = "${missing}".`,
    `Retry frame: ${before} -> (${missing}) -> ${after}.`,
  ].join("\n");
}
...
export function repairHintText(repairScaffold, level) {
  const missingOperation = repairScaffold.missing_operation;
  const before = repairScaffold.before;
  const after = repairScaffold.after;
  if (level === 1) {
    return `Tiny hint: focus only on "${missingOperation}". In one sentence, explain how that operation changes ${before} into ${after}.`;
  }
  if (level === 2) {
    return [
      "Guided hint: use this frame in your own words.",
      `${before} -> (${missingOperation}) -> ${after}`,
      "Now write one short sentence for the middle move.",
    ].join("\n");
  }
  return [
    "Stronger hint: this is a shape, not the answer.",
    `When ${before} is true, the key operation (${missingOperation}) drives the shift toward ${after}.`,
    "Now restate that causal shift in your own words.",
  ].join("\n");
}

export function recoveryNextActionText(repairScaffold) {
  const missingOperation =
    repairScaffold?.missing_operation || "the missing operation";
  return `Next best step: run /redrill and explain "${missingOperation}" in one sentence using before -> operation -> after.`;
}
