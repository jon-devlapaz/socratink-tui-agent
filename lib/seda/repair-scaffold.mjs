...
function hingeFromGapDescription(gapDescription, firstNode) {
  ...
  return "the step that connects them";
}
...
function buildOrientOpening(evaluation, firstNode, scaffold) {
  ...
  if (gap && isQuestionShaped(gap)) {
    return clampLearnerCopy(finishQuestion(gap));
  }
  if (gap && !isQuestionShaped(gap)) {
    return clampLearnerCopy(
      finishQuestion(`What has to happen — ${gap}`),
    );
  }
  if (isQuestionShaped(hinge)) {
    return clampLearnerCopy(
      finishQuestion(`The first time versus later — ${hinge}`),
    );
  }
  return clampLearnerCopy(
    finishQuestion(`The first time versus later — what had to happen — ${hinge}`),
  );
}
...
export function buildContingentProbe({ repairText = "", scaffold = {} } = {}) {
  ...
  if (snippet && !isInstructorFacing(snippet)) {
    ...
    return clampLearnerCopy(
      finishQuestion(`You mentioned ${snippet} — what had to happen — ${hinge}`),
    );
  }
  if (isQuestionShaped(hinge)) {
    return clampLearnerCopy(finishQuestion(`What had to happen — ${hinge}`));
  }
  return clampLearnerCopy(
    finishQuestion(`What had to happen — ${hinge}`),
  );
}
