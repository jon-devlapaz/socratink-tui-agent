...
import { evaluateLoopRubric, renderLoopRubricMarkdown } from "./loop-rubric.mjs";
...
export function studentProviderLabel() {
  ...
  if (provider === "openai_compatible") {
    const target = process.env.PERSONA_LLM_TARGET || "lmstudio";
    const model = compatibleStudentModel();
    return `${target}:${model}`;
  }
  return "cloud:gemini";
}

export function compatibleTargetForChoice(choice) {
...
export function isOpenAiCompatibleMode(choice) {
  return ["lmstudio", "router"].includes(compatibleTargetForChoice(choice));
}

export function lmStudioDefaultModel(env = process.env) {
  return (
    String(env.LM_STUDIO_MODEL || "").trim() ||
    (env.PERSONA_LLM_TARGET === "lmstudio"
      ? String(env.PERSONA_LLM_MODEL || "").trim()
      : "") ||
    "google/gemma-4-12b"
  );
}
...
export function compatibleDefaultModel(env = process.env) {
  return (
    (env.PERSONA_LLM_TARGET === "router"
      ? String(env.PERSONA_LLM_MODEL || "").trim()
      : "") ||
    String(env.LLM_OPENAI_COMPAT_MODEL || "").trim() ||
    "auto"
  );
}

export function compatibleStudentModel(env = process.env) {
  const target = compatibleTargetForChoice(env.PERSONA_LLM_TARGET || "router");
  if (target === "lmstudio") return lmStudioDefaultModel(env);
  return compatibleDefaultModel(env);
}
...
export function applyStudentProvider(student) {
  ...
  if (target === "lmstudio") {
    ...
    process.env.PERSONA_LLM_TARGET = "lmstudio";
    process.env.PERSONA_LLM_BASE_URL =
      process.env.LM_STUDIO_BASE_URL || "http://127.0.0.1:1234/v1";
    process.env.PERSONA_LLM_API_KEY = process.env.LM_STUDIO_API_KEY || "lm-studio";
    process.env.PERSONA_LLM_MODEL = lmStudioDefaultModel(process.env);
    return;
  }
  if (target === "router") {
    process.env.PERSONA_LLM_PROVIDER = "openai_compatible";
    process.env.PERSONA_LLM_TARGET = "router";
    process.env.PERSONA_LLM_BASE_URL =
      process.env.LLM_ROUTER_BASE_URL || process.env.LLM_BASE_URL || "";
    process.env.PERSONA_LLM_API_KEY =
      process.env.LLM_ROUTER_API_KEY || process.env.LLM_API_KEY || "";
    process.env.PERSONA_LLM_MODEL = compatibleDefaultModel(process.env);
    return;
  }
  if (target === "cloud") {
  ...
  throw new Error(`student provider must be cloud, local, lmstudio, or router; got: ${student}`);
}
...
export async function probeLmStudio({
  ...
  baseUrl =
    target === "router"
      ? process.env.LLM_ROUTER_BASE_URL || process.env.PERSONA_LLM_BASE_URL
      : process.env.PERSONA_LLM_BASE_URL || "http://127.0.0.1:1234/v1",
  apiKey =
    target === "router"
      ? process.env.LLM_ROUTER_API_KEY || process.env.PERSONA_LLM_API_KEY
      : process.env.PERSONA_LLM_API_KEY,
  model = target === "router" ? compatibleDefaultModel() : lmStudioDefaultModel(),
} = {}) {
  ...
  if (!ids.some((id) => id === model || id.includes(model) || model.includes(id))) {
    throw new Error(
      `OpenAI-compatible endpoint is up but model "${model}" not listed. ` +
        `Available: ${ids.join(", ") || "(none)"}`,
    );
  }
  return { target, baseUrl, model, models: ids };
}
...
export async function preflightPersonaRun({
  ...
  if (provider === "openai_compatible" || isOpenAiCompatibleMode(student)) {
    await probeLmStudio({
      target: compatibleTargetForChoice(process.env.PERSONA_LLM_TARGET || student || "lmstudio"),
      baseUrl: process.env.PERSONA_LLM_BASE_URL,
      apiKey: process.env.PERSONA_LLM_API_KEY,
      model: process.env.PERSONA_LLM_MODEL,
    });
...
export function compactLabEventTail(events) {
  return (events || []).map((event) => {
    ...
    for (const key of ["action", "error", "message", "duration_ms", "timeout_ms"]) {
...
export function personaTurn(profile, session, { repoRoot = REPO_ROOT } = {}) {
  ...
  const result = spawnSync(python, [PERSONA_SCRIPT], {
    ...
    input: JSON.stringify({
      ...
      awaiting_text: session.awaiting?.ctaText || session.awaiting?.fallback || "",
...
export async function runPersonaSession({
  ...
  while (!session.complete && !session.caseComplete && turns < maxTurns) {
    ...
    turns += 1;
    if (shouldStopAfterTurn?.({ session, turnRecord, eventsTail })) break;
  }
...
export function writePersonaArtifacts({
  ...
  const rubric = evaluateLoopRubric({ log, health, sessionRecord });
  const rubricPath = path.join(outDir, "loop-rubric.json");
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `- Loop rubric: ${path.basename(rubricPath)}`,
      "",
      "## Event types",
      "",
      ...
      "",
      renderLoopRubricMarkdown(rubric),
      "",
    ].join("\n"),
  );
