...
It is the substrate contract; product vocabulary lives in
`.mex/context/product-vocabulary.md`.

**Throughline:** facts in, phase out, handlers do not route.
Everything below expands that contract; if a change violates the throughline, it is wrong
even when tests pass.

**Agent workflow:** merge checklist and release ladder live in
`.mex/context/release-ladder.md`.

## Layers

| Layer | Responsibility | Artifacts |
| --- | --- | --- |
| **Strategy** | Graph honesty, phase catalog, canon boundaries | `.mex/context/graph-honesty.md`, `.mex/context/product-vocabulary.md`, prompt versions in `prompt_templates.py` |
...
| **Observability** | Read-only truth over runs | `session.json`, harness replay, harness dashboard, `lib/observability/dashboard-metrics.mjs` |
...
Moss’s **Fact → Audit → Broadcast** matches the Socratink throughline:
...
| **Graph-neutral** | Telemetry / routing only; no evidence mutation | `cold_help_turn`, `repair_dialogue_turn`, `post_bridge_transfer_check`, recovery events — see `.mex/context/graph-honesty.md` |
...
| harness replay command | Assert `expected_invariants` on promoted cases (`learning_cases/cases.jsonl`) |
| harness routing-proof command | Verify `nextPhase` routes each promoted trace (`lib/seda/routing-proofs.mjs`) |
| harness dashboard JSON command | Founder summaries for dogfooding |
...
Over-decomposed subnodes are **fake dungeon rooms**; the extract prompt owns the
current guardrail wording.

## Graph completion (session vs KC)

Vocabulary for **Case Complete**, **Session complete**, and graph **solidified**
lives in `.mex/context/product-vocabulary.md`. Three different “done” signals — do not conflate them in
...
Merge checklist and gates: `.mex/context/release-ladder.md`.
...
Graph-honesty rules live in `.mex/context/graph-honesty.md`. Scripted fixtures live under
`fixtures/`.

## Bridge function catalog

Wire contracts for the six `bridge.py` actions (template version, I/O schema,
emitted events, routing fields read by `nextPhase`) live in
...
- `.mex/ROUTER.md` — agent context router and scaffold entrypoint
