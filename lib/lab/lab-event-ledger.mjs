...
import { dialogueFromReport } from "./lab-dialogue.mjs";
...
export function projectLabBatchSnapshot(snapshot = {}) {
  ...
  const liveDialogue = snapshot.dialogue;
  const dialogue =
    Array.isArray(liveDialogue?.runs) && liveDialogue.runs.some((run) => (run.turns || []).length)
      ? liveDialogue
      : dialogueFromReport(snapshot.report);
  return {
    ...publicSnapshot,
    monitor,
