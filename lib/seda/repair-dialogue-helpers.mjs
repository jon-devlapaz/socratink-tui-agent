...
export function isQuestionLikeRepair(text) {
...
export function uncertaintyRecoveryPrompt(repairScaffold, step) {
  ...
  if (step === 1) {
    return `That's okay. Repair one missing link: explain ${hinge} in your own words.`;
  }
...
export function repairNudge(turnIndex, state, judge) {
  ...
  const emphasis = judge?.echo_risk
    ...
    : "Use your own words.";
  if (state?.escalationLevel >= 2 || turnIndex >= 2) {
    const finalPass = [
  ...
  if (state?.escalationLevel >= 1) {
    const analogical = [
      ...
      "Use the contrast you were given; name the missing link between those moments.",
    ];
...
export function questionRepairRedirect(repairScaffold) {
  const rawAfter = String(repairScaffold?.after || "").trim();
  const after =
    !rawAfter ||
    /correctly explained/i.test(rawAfter) ||
    /^(what|how|why)\b/i.test(rawAfter)
      ? "the outcome changes"
      : rawAfter;
  return (
    `Don't ask another question yet. Make one claim: ` +
    `"Later, ${after} because ___." Fill the blank in your own words.`
  );
}

export function repairFollowUpPrompt({ repair, judge, scaffold } = {}) {
  if (isQuestionLikeRepair(repair)) return questionRepairRedirect(scaffold);
  return (
    judge?.next_prompt ||
...
export function missingOperationFeedback(repairScaffold, judge) {
  ...
  return `Still not bridge-ready: ${hinge}. What connects ${before} to ${after}?`;
}
...
export function repairHintText(repairScaffold, level) {
  ...
  const contrast = repairScaffold.contrast_prompt;
  if (level === 1) {
    return `Tiny hint: focus on "${hinge}". What connects ${before} to ${after}?`;
  }
  if (level === 2) {
    const lines = [
...
export function recoveryNextActionText(repairScaffold) {
  const hinge = hingeLabel(repairScaffold);
  return `Next best step: when you're ready, repair one missing link: explain "${hinge}" in one sentence.`;
}
...
export function repairDialogueEvent({
  ...
  const event = eventBuilders.repairDialogueTurn({
    gap_id: gapId,
