...
export function assertSafeRequestedCommand(args) {
  ...
  for (const dangerous of DANGEROUS_COMMANDS) {
    if (dangerous.every((part, index) => args[index] === part)) {
      throw new CommandError(
        `blocked dangerous command: ${dangerous.join(" ")}. Use agent-git rescue first, then run destructive git manually if still needed.`,
        { code: 2 },
      );
    }
  }
...
export function doctor(cwd = process.cwd()) {
  ...
  if (ownerRepo && base && ghAuth.ok) {
    ...
    const protection = runStatus("gh", ["api", `repos/${ownerRepo}/branches/${base}/protection`], {
      cwd: root,
    });
    checks.push({
