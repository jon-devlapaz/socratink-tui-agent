...
export function rescue(cwd = process.cwd(), { message = "" } = {}) {
  ...
  fs.writeFileSync(path.join(dir, "worktree.patch"), checkedRun(["diff", "--binary"]) + "\n");
  fs.writeFileSync(path.join(dir, "index.patch"), checkedRun(["diff", "--staged", "--binary"]) + "\n");
  fs.writeFileSync(
    path.join(dir, "untracked-files.txt"),
    checkedRun(["ls-files", "--others", "--exclude-standard"]) + "\n",
  );
