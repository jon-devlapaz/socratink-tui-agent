...
function computeProductMetrics(runs, sessions) {
  ...
  return {
    meaningful_cold_attempt_rate: metric({
      ...
      sourceEventTypes: ["cold_attempt_submitted", "cold_attempt_evaluated"],
      formulaLabel:
    ...
    bridge_reach_rate: metric({
      ...
      sourceEventTypes: ["bridge_prompted"],
      formulaLabel: "sessions that reached the model bridge / sessions started",
    }),
    case_complete_rate: metric({
      numerator: canonicalBySession.filter((events) =>
        hasAnyScoreEligibleCanonical(events, [
          "case_completed",
          "spaced_redrill_submitted",
        ]),
      ).length,
      sourceEventTypes: ["case_completed", "spaced_redrill_submitted"],
      formulaLabel:
        "sessions with a completed spaced re-drill / sessions started",
    }),
    repair_load_rate: metric({
      numerator: canonicalBySession.filter((events) =>
        hasAnyCanonical(events, ["repair_prompted", "repair_submitted"]),
      ).length,
      sourceEventTypes: ["repair_prompted", "repair_submitted"],
      formulaLabel: "sessions that entered repair / sessions started",
    }),
    evidence_hold_rate: metric({
      numerator: canonicalBySession.filter(hasEvidenceHoldFromCanonicalEvents)
        .length,
      sourceEventTypes: [
      ...
      formulaLabel:
        "sessions with solid spaced reconstruction held below solidified / sessions started",
    }),
    substrate_seed_use_rate: metric({
      numerator: canonicalBySession.filter((events) =>
        hasCanonical(events, "substrate_seed_shown"),
      ).length,
      sourceEventTypes: ["substrate_seed_shown"],
      formulaLabel: "sessions shown a substrate seed / sessions started",
    }),
    meta_use_rate: metric({
      numerator: canonicalBySession.filter((events) =>
        hasAnyCanonical(events, ["meta_requested", "meta_returned"]),
      ).length,
      sourceEventTypes: ["meta_requested", "meta_returned"],
      formulaLabel: "sessions using /meta / sessions started",
    }),
  };
}
...
function hasAnyCanonical(events, eventTypes) {
  return eventTypes.some((eventType) => hasCanonical(events, eventType));
}
...
function hasAnyScoreEligibleCanonical(events, eventTypes) {
  return eventTypes.some((eventType) =>
    hasScoreEligibleCanonical(events, eventType),
  );
}

function classificationFor(event) {
  return event?.payload?.evaluation?.classification || event?.payload?.classification;
}

function hasEvidenceHoldFromCanonicalEvents(events) {
  const coldEvent = events.find(
    (event) =>
      event.event_type === "cold_attempt_evaluated" &&
      event.score_eligible === true,
  );
  const coldClassification = classificationFor(coldEvent);
  if (!coldClassification || coldClassification === "solid") return false;
  return events.some(
    (event) =>
      event.event_type === "spaced_redrill_submitted" &&
      event.score_eligible === true &&
      classificationFor(event) === "solid",
  );
}
