...
export async function handleRepairAbandoned({ events, derived, store, ctx }) {
  ...
  const reason = lastTurn?.uncertainty
    ...
    : lastTurn?.next_dialogue_action === "abandon"
      ? "dialogue_abandoned"
      : "unresolved_gap";
  events.push(eventBuilders.repairStateBucketed({
  ...
  events.push(eventBuilders.repairCapSelected({
    bucket,
    cap_value: capValue,
  ...
  events.push(eventBuilders.repairRecoveryStarted({
  ...
  events.push(eventBuilders.repairAbandoned({
    text: lastTurn?.text || "",
    reason,
