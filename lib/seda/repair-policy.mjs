...
export function applyRepairTurnBudget(
  judge,
  { turnIndex, maxRepairTurns, repairText = "", previousRepairText = "" },
) {
  if (judge.bridge_ready || turnIndex < maxRepairTurns) return judge;
  if (judge.next_dialogue_action === "abandon") return judge;
  const questionAtCap = /\?\s*$/.test(String(repairText).trim());
  const normalizedRepair = String(repairText).trim().toLowerCase();
  const normalizedPrevious = String(previousRepairText).trim().toLowerCase();
  const distinctFinalTry =
    normalizedRepair && normalizedPrevious && normalizedRepair !== normalizedPrevious;
  if (
    !questionAtCap &&
    !distinctFinalTry &&
    !judge.causal_link_present &&
    !judge.missing_operation_addressed
  ) {
    return judge;
  }
  return {
