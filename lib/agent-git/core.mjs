...
function remoteBranches(root) {
  return execQuiet("git", ["branch", "-r", "--format=%(refname:short)"], { cwd: root })
    .split("\n")
...
export function goldenStatus(cwd = process.cwd(), { gh = true } = {}) {
  const root = repoRoot(cwd);
  const base = defaultBranch(root) || "main";
  const branch = currentBranch(root);
  const porcelain = execQuiet("git", ["status", "--porcelain=v1"], { cwd: root });
  const aheadBehind = runStatus("git", ["rev-parse", "--verify", `origin/${base}`], { cwd: root }).ok
    ? execQuiet("git", ["rev-list", "--left-right", "--count", `origin/${base}...HEAD`], { cwd: root })
    : null;
  const [behind = "unknown", ahead = "unknown"] = aheadBehind ? aheadBehind.split(/\s+/) : [];
  const currentRoot = fs.realpathSync(root);
  const extraWorktrees = worktreeEntries(root).filter((entry) => {
    try {
      return fs.realpathSync(entry.path) !== currentRoot;
    } catch {
      return true;
    }
  });
  const localAgentBranches = localBranches(root).filter((name) => /^(agent|codex)\//.test(name));
  const remoteAgentBranches = remoteBranches(root).filter((name) => /^(agent|codex)\//.test(name));
  const stashes = lines(runStatus("git", ["stash", "list"], { cwd: root }).stdout);
  ...
  if (gh) {
    const prResult = runStatus(
      "gh",
      ["pr", "list", "--state", "open", "--limit", "100", "--json", "number,headRefName,title,url"],
      { cwd: root },
    );
    ghAvailable = prResult.ok;
    if (prResult.ok && prResult.stdout) openPrs = JSON.parse(prResult.stdout);
  }

  const blockers = [];
  if (branch !== base) blockers.push(`not on ${base}: ${branch}`);
  if (porcelain) blockers.push("dirty worktree");
  if (aheadBehind && (behind !== "0" || ahead !== "0")) blockers.push(`origin/${base} mismatch: behind ${behind}, ahead ${ahead}`);
  if (!aheadBehind) blockers.push(`missing origin/${base}`);
  if (ghAvailable === false) blockers.push("open PR check unavailable");
  if (openPrs.length) blockers.push(`${openPrs.length} open PR(s)`);
  if (extraWorktrees.length) blockers.push(`${extraWorktrees.length} extra worktree(s)`);
  if (localAgentBranches.length) blockers.push(`${localAgentBranches.length} local agent/codex branch(es)`);
  if (remoteAgentBranches.length) blockers.push(`${remoteAgentBranches.length} remote agent/codex branch(es)`);
  if (stashes.length) blockers.push(`${stashes.length} stash entry/entries`);

  return {
    root,
    base,
    branch,
    golden: blockers.length === 0,
    blockers,
    dirty: porcelain.length > 0,
    porcelain,
    aheadBehind,
...
export function rescue(cwd = process.cwd(), { message = "" } = {}) {
  ...
  if (!porcelain) {
    return {
      ...
      rescueDir: null,
      stashed: false,
      stashOutput: "Worktree is clean; rescue skipped.",
    };
  }

  const checkedRun = (args) => {
    const result = runStatus("git", args, { cwd: root });
    if (!result.ok) {
      throw new CommandError(`git ${args.join(" ")} failed`, {
        code: result.code || 1,
  ...
  const statusText = execQuiet("git", ["status", "--short", "--branch"], { cwd: root });
  const worktreePatch = checkedRun(["diff", "--binary"]);
  const indexPatch = checkedRun(["diff", "--staged", "--binary"]);
  const untrackedFiles = checkedRun(["ls-files", "--others", "--exclude-standard"]);

  fs.mkdirSync(dir, { recursive: true });

  fs.writeFileSync(path.join(dir, "status.txt"), statusText + "\n");
  fs.writeFileSync(path.join(dir, "worktree.patch"), worktreePatch + "\n");
  fs.writeFileSync(path.join(dir, "index.patch"), indexPatch + "\n");
  fs.writeFileSync(path.join(dir, "untracked-files.txt"), untrackedFiles + "\n");

  const stashMessage = `agent-git rescue ${stamp}${message ? `: ${message}` : ""}`;
  const stashOutput = checkedRun(["stash", "push", "--include-untracked", "-m", stashMessage]);
