...
async function probeChatCompletion({ baseUrl, apiKey, model, timeoutMs = 5_000 }) {
  ...
  try {
    ...
    if (err?.name === "AbortError") {
      throw new Error(`Model probe timed out after ${timeoutMs}ms`);
    }
    throw err;
  } finally {
    clearTimeout(timer);
...
export async function handleLabApi(req, res, url, options = {}) {
  ...
  if (req.method === "GET" && url === "/api/lab/runs") {
    json(res, 200, { runs: runStore.listLabRuns?.() || [] });
    return true;
  }

  const dialogueMatch = url.match(/^\/api\/lab\/runs\/([^/]+)\/dialogue$/);
  if (req.method === "GET" && dialogueMatch) {
    const payload = runStore.getLabRunDialogue?.(dialogueMatch[1]);
    if (!payload) {
      json(res, 404, { error: "run dialogue not found" });
      return true;
    }
