  ...
  return {
    ...
    product_strategy_v2: {
      ...
      north_star: {
        ...
        primary_rate: productMetrics.case_complete_rate,
      },
...
function computeProductMetrics(runs, sessions) {
  const total = sessions.length || runs.length || 0;
  const countSessions = (predicate) => sessions.filter(predicate).length;
  const countRuns = (predicate) => runs.filter(predicate).length;

  return {
    meaningful_cold_attempt_rate: rate(
      countSessions((session) => eventTypes(session).includes("cold_attempt")),
      total,
    ),
    bridge_reach_rate: rate(
      countSessions((session) => eventTypes(session).includes("model_bridge")),
      total,
    ),
    case_complete_rate: rate(
      countSessions((session) => eventTypes(session).includes("spaced_redrill")),
      total,
    ),
    repair_load_rate: rate(
      countRuns((run) => run.friction.includes("repair load")),
      runs.length,
    ),
    evidence_hold_rate: rate(
      countRuns((run) => run.evidence_holds > 0),
      runs.length,
    ),
    substrate_seed_use_rate: rate(
      countSessions((session) =>
        eventTypes(session).includes("substrate_seed_offered"),
      ),
      total,
    ),
    meta_use_rate: rate(
      countSessions((session) => eventTypes(session).includes("meta_turn")),
      total,
    ),
  };
}
