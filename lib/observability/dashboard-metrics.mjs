import { deriveProductLoopBranch } from "../seda/session-record.mjs";
import { canonicalEventsForSession } from "../seda/event-taxonomy.mjs";
import { EVENT_FACT_DEFINITIONS } from "../seda/event-facts.mjs";
...
export function buildDashboardPayload({ cases, sessions }) {
  ...
  const runs = cases.map((caseRecord) =>
    buildRunLog(caseRecord, sessionByLog.get(caseTrace(caseRecord)) ?? {}),
  );
  ...
  return {
    ...
    case_summary: {
      total: cases.length,
      regression: cases.filter((c) => caseKind(c) === "regression").length,
      golden: cases.filter((c) => caseKind(c) === "golden").length,
      research: cases.filter((c) => caseKind(c) === "research").length,
    },
...
function indexSessionsByCaseLog(cases, sessions) {
  ...
  for (const caseRecord of cases) {
    const trace = caseTrace(caseRecord);
    if (!trace) continue;
    if (sessionIndex >= sessions.length) break;
    sessionByLog.set(trace, sessions[sessionIndex]);
    sessionIndex += 1;
  }
  return sessionByLog;
}

function caseKind(caseRecord) {
  return caseRecord.kind || caseRecord.case_type;
}

function caseSource(caseRecord) {
  return caseRecord.source || caseRecord.case_source;
}

function caseTrace(caseRecord) {
  return caseRecord.trace || caseRecord.session_log;
}

function caseChecks(caseRecord) {
  return caseRecord.checks || caseRecord.expected_invariants || {};
}
...
function buildDogfoodEvidence(cases, sessions) {
  const sourceCounts = {};
  for (const caseRecord of cases) {
    const source = caseSource(caseRecord) || "unknown";
...
function computeTraceability(cases, sessions) {
  const casesWithInvariants = cases.filter((caseRecord) =>
    Boolean(caseRecord.checks || caseRecord.expected_invariants),
  ).length;
  const casesWithSessionLogs = cases.filter((caseRecord) =>
    Boolean(caseTrace(caseRecord)),
  ).length;
...
function buildRunLog(caseRecord, session = {}) {
  ...
  const dialogueTurns = events.filter((event) => event.type === "repair_dialogue_turn");
  const finalState =
    caseChecks(caseRecord).final_node_state ||
  ...
  return {
    ...
    type: caseKind(caseRecord),
    source: caseSource(caseRecord),
    concept: caseRecord.concept || session.concept || "Unknown concept",
    product_question: caseRecord.product_question,
    claim: caseRecord.claim || caseRecord.expected_invariant,
    protected_failure: caseRecord.risk || caseRecord.observed_failure,
    expected_invariant:
      caseRecord.claim ||
      caseRecord.expected_invariant ||
      summarizeInvariant(caseChecks(caseRecord)),
    ...
    cold_classification:
      ...
      caseChecks(caseRecord).cold_evaluator_classification ||
      "not scored",
    spaced_classification:
      spacedAttempt?.evaluation?.classification ||
      caseChecks(caseRecord).spaced_evaluator_classification ||
      "not reached",
