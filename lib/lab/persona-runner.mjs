...
export function isOpenAiCompatibleMode(value) {
...
export function compatibleTargetForChoice(value) {
  const mode = String(value || "").trim().toLowerCase();
  if (mode === "local" || mode === "lmstudio" || mode === "lm_studio") return "lmstudio";
  return "router";
}

export function compatibleDefaultModel(env = process.env) {
  return endpointDefaultModel("router", env);
}

export function lmStudioDefaultModel(env = process.env) {
  return endpointDefaultModel("lmstudio", env);
}

export function compatibleStudentModel(env = process.env) {
  return compatibleModelForTarget(env.PERSONA_LLM_TARGET || "router", env);
}

function compatibleBaseUrl(target = "router", env = process.env) {
  return endpointBaseUrl(normalizeCompatTarget(target), env);
}

function compatibleApiKey(target = "router", env = process.env) {
  return endpointApiKey(normalizeCompatTarget(target), env);
}

function compatibleModelForTarget(target = "router", env = process.env) {
  const normalizedTarget = normalizeCompatTarget(target);
  const configuredTarget = env.PERSONA_LLM_TARGET
    ? normalizeCompatTarget(env.PERSONA_LLM_TARGET)
    : null;
  const configuredModel = String(env.PERSONA_LLM_MODEL || "").trim();
  if (configuredModel && configuredTarget === normalizedTarget) return configuredModel;
  return endpointDefaultModel(normalizedTarget, env);
}
...
export function studentProviderLabel() {
  ...
  if (provider === "openai_compatible") {
    return `compatible:${compatibleStudentModel()}`;
  }
  return `cloud:${process.env.PERSONA_GEMINI_MODEL || "gemini-2.5-flash"}`;
}

export function applyStudentProvider(student) {
  if (isOpenAiCompatibleMode(student)) {
    const target = compatibleTargetForChoice(student);
    process.env.PERSONA_LLM_PROVIDER = "openai_compatible";
    process.env.PERSONA_LLM_TARGET = target;
    process.env.PERSONA_LLM_BASE_URL = compatibleBaseUrl(target);
    process.env.PERSONA_LLM_MODEL = compatibleModelForTarget(target);
    process.env.PERSONA_LLM_API_KEY = compatibleApiKey(target);
    return;
  }
  if (student === "cloud") {
  ...
  throw new Error(`student provider must be "cloud" or "compatible", got: ${student}`);
}
...
export async function probeLmStudio({
  target = process.env.PERSONA_LLM_TARGET || "lmstudio",
  baseUrl = compatibleBaseUrl(target),
  apiKey = compatibleApiKey(target),
  model = compatibleModelForTarget(target),
} = {}) {
  ...
  if (model === "auto") return { baseUrl, model, models: ids };
  if (!ids.some((id) => id === model || id.includes(model) || model.includes(id))) {
    throw new Error(
      `OpenAI-compatible endpoint is up but model "${model}" not listed. Available: ${ids.join(", ") || "(none)"}`,
    );
  }
...
export async function preflightPersonaRun({
  ...
  const provider = (process.env.PERSONA_LLM_PROVIDER || "gemini").trim().toLowerCase();
  if (provider === "openai_compatible" || isOpenAiCompatibleMode(student)) {
    await probeLmStudio();
  } else if (!(process.env.GEMINI_API_KEY || "").trim()) {
    throw new Error(
      "Cloud student brain requires GEMINI_API_KEY in .env or shell (or use --student compatible).",
    );
  }
...
function fakeConceptToken(profile) {
  return String(profile?.concept || "").trim().toLowerCase();
}

function fakeUsesImmuneRoute(profile) {
  const token = fakeConceptToken(profile);
  return /immune|vaccine|antigen|antibody|immunity/.test(token);
}

function fakeUsesCacheRoute(profile) {
  const token = fakeConceptToken(profile);
  if (/cache|caching|redis|memoiz/.test(token)) return true;
  return /memoization|memoize|memoized|memoizing/.test(token);
}

export function fakeFallback(session, { allowFake, health, profile }) {
  if (!allowFake || !health.fake_llm) return null;
  const key = session.awaiting?.key;
  if (key === "substrate_refinement") {
    return (
      profile?.substrate_refinement ||
      "Vaccines give the body a safe preview so it can respond faster later."
    );
  }
  if (key === "cold_attempt" || key === "spaced_attempt" || key === "gap_attempt") {
    if (fakeUsesImmuneRoute(profile)) {
      return (
        "A safe vaccine preview leaves memory cells behind, so a later antigen match triggers a faster immune response."
      );
    }
    if (fakeUsesCacheRoute(profile)) {
      return (
    ...
    return (
      "An earlier encounter leaves a trace, and a later encounter can build on that trace instead of starting from zero."
    );
  }
  return null;
}
...
function brainBanner(health, { allowFake, llm = null }) {
  const tutor = health.fake_llm
    ? "sandbox"
    : llm?.provider === "openai_compatible"
      ? `compatible:${llm.model}`
      : `live:${llm?.model || health.llm_model || "gemini-2.5-flash"}`;
  const student = studentProviderLabel();
  return `tutor=${tutor} student=${student} allow_fake=${allowFake}`;
}

function bridgeDiagnosticsFromEvents(events = []) {
  return events
    .filter((event) => event?.type === "bridge_error")
    .map((event) => ({
      ...
      diagnostic_id: event.diagnostic_id || null,
      diagnostic_path: event.diagnostic_path || null,
    }));
}

export function compactLabEventTail(events = []) {
  ...
  return events
    .map((event) => {
      ...
      for (const key of ["action", "error", "message"]) {
        if (event[key]) compact[key] = String(event[key]);
      }
      for (const key of ["duration_ms", "timeout_ms"]) {
        if (Number.isFinite(Number(event[key]))) compact[key] = Number(event[key]);
      }
      return compact;
    })
    .filter(Boolean);
...
export async function runPersonaSession({
  ...
  let session = await fetchJson(`${baseUrl}/api/session`, {
    method: "POST",
  ...
  const log = {
    ...
    persona_hint: profile.persona_hint,
    brains: brainBanner(health, { allowFake, llm }),
    llm_mode: health.llm_mode,
    bridge_diagnostics_dir: session.bridge_diagnostics_dir || null,
    turns: [],
  };
  ...
  while (!session.complete && !session.caseComplete && turns < maxTurns) {
    ...
    if (transportContinue) {
      ...
      if (text) {
        ...
        text = fakeFallback(session, { allowFake, health, profile });
        if (text) kind = "fake";
      }
    }
    ...
    const eventsTail = compactLabEventTail(session.events || []);
    onTurnComplete?.({
      turnRecord,
    ...
    log.turns.push({
      ...
      bridge_errors: bridgeDiagnosticsFromEvents(session.events).slice(-3),
      transcript_delta: session.transcript,
    });
    turns += 1;
  }
  ...
  log.final = {
    ...
    case_complete: session.caseComplete,
    event_types: (session.events || []).map((e) => e.type),
    bridge_errors: bridgeDiagnosticsFromEvents(session.events),
    bridge_diagnostics_dir: session.bridge_diagnostics_dir || log.bridge_diagnostics_dir || null,
...
export function writePersonaArtifacts({
  ...
  if (sessionRecord) {
    ...
    rubric = evaluateLoopRubric({ log, health, sessionRecord });
    rubricPath = path.join(outDir, "loop-rubric.json");
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `- Session complete (hosted): ${log.final.complete}`,
      `- Case complete (pedagogy): ${log.final.case_complete}`,
      `- Hit max turns: ${log.final.hit_max_turns}`,
      `- Final phase: ${log.final.phase}`,
      ...(log.final.bridge_diagnostics_dir
        ? [`- Bridge diagnostics: ${log.final.bridge_diagnostics_dir}`]
        : []),
      ...(sessionPath ? [`- Session record: ${path.basename(sessionPath)}`] : []),
      ...(rubricPath ? [`- Loop rubric: ${path.basename(rubricPath)}`] : []),
      "",
      ...(log.final.bridge_errors?.length
        ? [
            "## Bridge errors",
            "",
            ...log.final.bridge_errors.map((error, index) => {
              const diagnostic = error.diagnostic_path
                ? ` — diagnostic: ${error.diagnostic_path}`
                : "";
              return `${index + 1}. ${error.action || "bridge"} (${error.phase || "unknown"}): ${error.message || error.error || "failed"}${diagnostic}`;
            }),
            "",
          ]
        : []),
      ...
      "",
      ...(rubric ? [renderLoopRubricMarkdown(rubric)] : []),
  ...
  return { reportPath, mdPath, sessionPath, rubricPath };
}
